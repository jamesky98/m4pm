...
function sizechange(e){
  emit('minichange', miniCaseBox.value)
}

function checkchange(e){
  emit('showsubcase', checkShow.value)
}
...
<template>
  <div 
    ...
    <div 
      ...
      <!-- 最小化符號 -->
      <label
        :class="['pos-absolute','curser-pointer',miniCaseBox?'max-btn':'mini-btn']">
        <input 
          type="checkbox" 
          class="sr-only" 
          v-model="miniCaseBox"
          @change="sizechange($event)"/>
        <i :class="['far',miniCaseBox?'fa-window-maximize':'fa-minus-square','transtition-all']"></i>
      </label>
      
      <!-- 展開符號 -->
      <label v-show="!props.isSubCase && props.caseData.other_m4case.length>0"
        :class="['pos-absolute','curser-pointer','close-btn']">
        <input 
          type="checkbox" 
          class="sr-only" 
          v-model="checkShow"
      ...
      <MDBBtn v-if="props.isSubCase"
        :class="['pos-absolute','curser-pointer','move-btn move-up']"
        @click.stop="moveSubCaseEvent(-1)"
        ><i class="fas fa-long-arrow-alt-up"></i></MDBBtn>
      <!-- 下移 -->
      <MDBBtn v-if="props.isSubCase"
        :class="['pos-absolute','curser-pointer','move-btn move-down']"
        @click.stop="moveSubCaseEvent(1)"
        ><i class="fas fa-long-arrow-alt-down"></i></MDBBtn>
    </div>
    <!-- 右方時間列表 -->
    <div v-show="!miniCaseBox" 
      :style="'width: calc(100% - ' + props.leftCaseWidth + 'rem);min-width: 4rem'">
      <div 
